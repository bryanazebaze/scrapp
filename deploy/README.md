...
sudo DASHSCOPE_API_KEY=sk-xxxx ./deploy/setup.sh
...
6. Generate `/opt/centralimmo/.env` (DB URL, `API_HOST=0.0.0.0`, CORS, DashScope key).
...
- **AI features silent** — `DASHSCOPE_API_KEY` is empty in `.env`. Re-run
  `sudo DASHSCOPE_API_KEY=sk-xxxx ./deploy/setup.sh`.